...
public class DeleteList extends Activity implements OnItemClickListener,OnCheckedChangeListener  
{
	ListView lv1;
...
String fn1,ln1;
	public void onCreate(Bundle savedInstanceState)
	{
		...
		data=new DataBaseHelper(this);
		 Bundle bundle = getIntent().getExtras(); 
			name3=bundle.getString("name2");
			System.out.println("List Name "+name3);
			id1=Integer.parseInt(name3);
			System.out.println("List Id "+id1);
		Window window = getWindow();
        window.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN);
		lv1=(ListView)findViewById(R.id.List);
		chkall=(CheckBox)findViewById(R.id.chk_all);
		Cursor c3=data.getconid_calllist(id1);
		while(c3.moveToNext())
		{
			cid2=c3.getInt(0);
	...
	private void makeSelections() 
	{
		...
		Cursor c=data.getconid_calllist(id1);
		while(c.moveToNext())
		{   
			cid4=c.getInt(0);
			System.out.println("cid will "+cid4);
			data.deleteCallListdetails(id1,cid4);
		}
		c.close();		
	...
	public void onItemClick(AdapterView<?> a, View v, int position, long id3) 
	{
		...
		while(c8.moveToNext())
		{
			...
			data.deleteCallListdetails(id1,cid1);
		}
