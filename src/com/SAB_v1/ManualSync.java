...
private int sms_flag1;
	...
	{	
		...
		stime=mHour+":"+mMinute;
		System.out.println("Current Time is:"+stime);
		...
		Cursor c=db1.get_sync_login_flag();
		while(c.moveToNext())
		{
			login_flag=c.getInt(0);
		}
		c.close();
		if(login_flag==1)
		{
		Cursor c1=db1.get_sync_login_id(login_flag);
		while(c1.moveToNext())
		{
		  login_id=c1.getInt(0);	
		Cursor c2=db1.get_sync_login_details(login_id);
		while(c2.moveToNext())
		{
			login_user=c2.getString(0);
			login_pass=c2.getString(1);
			System.out.println("Login id,User,Pass "+login_id+" "+login_user+""+login_pass);
		}
		c2.close();
		}
		c1.close();
			uname.setText(login_user);
			pwd.setText(login_pass);
			chk.setChecked(true);
		}
		initControls();
	...
	private void checkValidate()
	{
		...
		if(chk.isChecked())
		 {
	        	db1.Insert_sync_login(uname.getText().toString(), pwd.getText().toString());
	        	Cursor c1=db1.get_sync_login_id_uname(uname.getText().toString(),pwd.getText().toString());
	    		while(c1.moveToNext())
	    		{
	    		  login_id1=c1.getInt(0);
	    		}c1.close();
	        	db1.Update_sync_login_flag(login_id1);
	      }
		...
		else
		{
    		...
    		task.applicationContext = ManualSync.this;
    		task.execute();*/
			login();
		}
	}
	 /*type :function
  	...
  	return type:void          
	date:10-2-12
  	purpose:sends data to server and logins to id*/
	private void login()
	{
...
System.out.println("task are== --"+tasko+" "+taskname+" "+taskdes+"  "+taskpri+" "+tasksdate+" "+taskddate+" "+tasktime);
		...
		{	
			...
			try{
				...
				db1.Insertnotes(con2, name, no1,note1,date1,time1);
